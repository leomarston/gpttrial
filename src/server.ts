...
if (!ALPHA_VANTAGE_API_KEY) {
  console.warn(
    'ALPHA_VANTAGE_API_KEY is not set. The topMovers tool will fail until you provide a valid API key.'
  );
}
...
type ToolOutput = z.infer<typeof toolOutputParser>;
...
const normalizeList = (entries: unknown[], limit: number): Instrument[] => {
  return entries
    .slice(0, limit)
...
const fetchTopMovers = async (rawInput: unknown): Promise<ToolOutput> => {
  if (!ALPHA_VANTAGE_API_KEY) {
    throw new Error(
      'Missing Alpha Vantage API key. Set ALPHA_VANTAGE_API_KEY before calling the topMovers tool.'
    );
  }
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Alpha Vantage request failed with status ${response.status}: ${body.substring(0, 200)}`
    );
  }
